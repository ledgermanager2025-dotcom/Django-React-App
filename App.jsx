...
const App = () => {
    ...
    const getCustomerFinancials = useCallback((customerId) => {
        ...
        const CustomerNetProfit = grossProfit - borrowings;
        return { 
                ...
                <div className="space-y-4">
                    ...
                    <div className="max-h-96 overflow-y-auto divide-y divide-gray-200 border rounded-lg">
                        <div className="flex bg-gray-50 font-semibold text-sm uppercase text-gray-600 sticky top-0">
                            <span className="p-3 w-1/4">Customer</span>
                            <span className="p-3 w-1/4 text-center">Revenue</span>
                            <span className="p-3 w-1/4 text-center">Gross Profit</span>
                            <span className="p-3 w-1/4 text-center">Net Profit</span>
                        </div>
                        {customerPnlData.length > 0 ? (
                            customerPnlData.map((c) => (
                                <div key={c.id} className="flex hover:bg-gray-50">
                                    <span className="p-3 w-1/4 text-sm text-gray-800 font-medium">{c.name}</span>
                                    <span className="p-3 w-1/4 text-sm text-center text-indigo-600">
                                        ₹{c.sellValue.toFixed(2)}
                                    </span>
                                    <span 
                                        className={c.grossProfit >= 0 ? 'p-3 w-1/4 text-sm text-center font-semibold text-green-600' : 'p-3 w-1/4 text-sm text-center font-semibold text-red-600'}
                                    >
                                        ₹{c.grossProfit.toFixed(2)}
                                    </span>
                                    <span className="p-3 w-1/4 text-sm
                                    text-green-600 text-center font-medium">₹{c.CustomerNetProfit}</span>
                                </div>
                            ))
                        ) : (
